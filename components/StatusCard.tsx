...
export const StatusCard: React.FC<StatusCardProps> = ({ icon, label, value, isHighlighted = false, tooltip }) => {
  ...
  const content = (
    <div className={`group relative bg-white/[0.01] backdrop-blur-3xl p-7 flex items-center space-x-6 transition-all duration-700 border rounded-[2.5rem] hover:bg-white/[0.03] hover:border-white/20 hover:shadow-[0_20px_45px_rgba(0,0,0,0.4)] ${highlightClass}`}>
      ...
      <div className="text-left flex-1 min-w-0">
        ...
        <p className="text-3xl font-black text-premium-gradient tracking-tighter uppercase italic group-hover:scale-[1.02] origin-left transition-all duration-500 truncate">
          {value}
        </p>
      </div>
    </div>
